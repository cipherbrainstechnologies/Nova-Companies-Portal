...
1. **Secrets** — `AUTH_SECRET`, `OTP_PEPPER`, database, Redis, S3/R2 (or Railway bucket), email API key. On Railway these are provisioned via `.railway/railway.ts` except secrets you set once in Variables.
...
7. **Worker PDF** — production worker image installs Chromium via the root Dockerfile (`npx playwright install --with-deps chromium`).
