...
- Payment auto-match is identity-first (`identity-matching.ts` + `payment-auto-match.ts`): beneficiary/alias/account evidence decides the employee before amount comparison. Equal amounts alone never auto-link. Shared runner is invoked on payroll create/populate, after statement parse commits, and via `POST /api/payroll/runs/[runId]/auto-match` (“Run Auto-Match Again”). Manual approvals are preserved. Default search window is −10 / +45 days around the salary month and is shown/adjustable in the reconciliation UI.
